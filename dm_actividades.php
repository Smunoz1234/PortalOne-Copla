...
//Actividades
$SQL_Actividades=Seleccionar('uvw_Sap_tbl_Actividades','
	   [ID_Actividad]
      ...
      ,[DeEstadoActividad]
	  ,[IdAsignadoPor]
      ,[DeAsignadoPor]',"[DocMarkDocEntry]='".$IdDocMarketing."' and [DocMarkDocType]='".$IdDocType."'","[ID_Actividad]","DESC");
			...
			<?php while($row_Actividades=sql_fetch_array($SQL_Actividades)){
				...
				?>
				 <tr>
					...
					<td><?php if($row_Actividades['FechaHoraInicioActividad']!=""){ echo $row_Actividades['FechaHoraInicioActividad'];}else{?><p class="text-muted">--</p><?php }?></td>
					<td><?php if($row_Actividades['FechaHoraFinActividad']!=""){ echo $row_Actividades['FechaHoraFinActividad'];}else{?><p class="text-muted">--</p><?php }?></td>
					<td><p class='<?php echo $DVenc[0];?>'><?php echo $DVenc[1];?></p></td>
					<td><?php if($row_Actividades['ID_OrdenServicioActividad']!=0){?><a href="llamada_servicio.php?id=<?php echo base64_encode($row_Actividades['ID_LlamadaServicio']);?>&tl=1&return=<?php echo base64_encode($_SERVER['QUERY_STRING']);?>&pag=<?php echo base64_encode('orden_venta.php');?>"><?php echo $row_Actividades['ID_OrdenServicioActividad'];?></a><?php }else{echo "--";}?></td>							
					...
					<td><a href="actividad.php?id=<?php echo base64_encode($row_Actividades['ID_Actividad']);?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']);?>&pag=<?php echo base64_encode('orden_venta.php');?>&tl=1" class="btn btn-link btn-xs" target="_blank"><i class="fa fa-folder-open-o"></i> Abrir</a></td>
				</tr>
			<?php }?>
			</tbody>
		</table>
