...
$dt_ET=0;//sw para saber si vienen datos de una Orden de venta.
...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			remote=open('buscar_articulo.php?dato='+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if($edit==0){echo "13";}else{echo "14";}?>&iddevolucionventa=<?php if($edit==1){echo base64_encode($row['ID_DevolucionVenta']);}else{echo "0";}?>&evento=<?php if($edit==1){echo base64_encode($row['IdEvento']);}else{echo "0";}?>&tipodoc=2','remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			swal({
				title: "¡Error!",
...
<script type="text/javascript">
	$(document).ready(function() {//Cargar los combos dependiendo de otros
		$("#CardCode").change(function(){
			...
			<?php }else{?>
				if(carcode!="" && almacen!=""){
					frame.src="detalle_devolucion_venta.php?id=<?php echo base64_encode($row['ID_DevolucionVenta']);?>&evento=<?php echo base64_encode($row['IdEvento']);?>&type=2";
				}else{
					frame.src="detalle_devolucion_venta.php";
				}
			<?php }?>		
			...
			<div class="col-lg-12">   		
				...
				<?php include("includes/spinner.php"); ?>
					<div class="form-group">
						...
						<div class="col-lg-6">
							<button type="button" onClick="javascript:location.href='actividad.php?dt_DM=1&Cardcode=<?php echo base64_encode($row['CardCode']);?>&Contacto=<?php echo base64_encode($row['CodigoContacto']);?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']);?>&Direccion=<?php echo base64_encode($row['DireccionDestino']);?>&DM_type=<?php echo base64_encode('15');?>&DM=<?php echo base64_encode($row['DocEntry']);?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']);?>&pag=<?php echo base64_encode('devolucion_venta.php');?>'" class="alkin btn btn-primary pull-right"><i class="fa fa-plus-circle"></i> Agregar actividad</button>
						</div>
						<?php }?>
					</div>
				</div>
           ...
           <div class="col-lg-12">
              <form action="devolucion_venta.php" method="post" class="form-horizontal" enctype="multipart/form-data" id="CrearDevolucionVenta">
				...
				<div class="col-lg-4">
					...
					<div class="form-group">
						<label class="col-lg-5">Fecha de devolucion/servicio</label>
						<div class="col-lg-7 input-group date">
				...
				<div class="tabs-container">  
					...
					<div class="tab-content">
						<div id="tab-1" class="tab-pane active">
							<iframe id="DataGrid" name="DataGrid" style="border: 0;" width="100%" height="300" src="<?php if($edit==0){echo "detalle_devolucion_venta.php";}else{echo "detalle_devolucion_venta.php?id=".base64_encode($row['ID_DevolucionVenta'])."&evento=".base64_encode($row['IdEvento'])."&docentry=".base64_encode($row['DocEntry'])."&type=2&status=".base64_encode($row['Cod_Estado']);}?>"></iframe>
						</div>
						<?php if($edit==1){?>
			   ...
			   <form id="frm" action="" class="form-horizontal">
				...
				<div class="form-group">
					<div class="col-lg-9">
						...
						<?php }?>
						<?php 
							$EliminaMsg=array("&a=".base64_encode("OK_DVenAdd"),"&a=".base64_encode("OK_DVenUpd"));//Eliminar mensajes
