...
if(isset($_POST['P'])&&($_POST['P']!="")){//Grabar Orden de venta
	...
	try{
		...
		if($SQL_CabeceraOrdenVenta){
			...
			try{
				$Parametros=array(
					'pIdOrdenVenta' => $IdOrdenVenta,
					'pIdEvento' => $IdEvento,
					'pLogin'=>$_SESSION['User']
				);
				$Metodo="AppPortal_InsertarOrdenVentaServicio";
				...
				if($Resultado['ID_Respuesta']=="0"){
					...
					if($_POST['P']==37){//Creando orden	
						...
						//$SQL_ConsID=Seleccionar('uvw_Sap_tbl_OrdenesVenta','ID_OrdenVenta',"IdDocPortal='".$IdOrdenVenta."'");
...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			remote=open('buscar_articulo.php?dato='+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if($edit==0){echo "1";}else{echo "2";}?>&idordenventa=<?php if($edit==1){echo base64_encode($row['ID_OrdenVenta']);}else{echo "0";}?>&evento=<?php if($edit==1){echo base64_encode($row['IdEvento']);}else{echo "0";}?>&tipodoc=2','remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			Swal.fire({
				title: "¡Error!",
				...
				<?php include("includes/spinner.php"); ?>
					<div class="form-group">
						...
						<?php if($row['Cod_Estado']!='C'){?>
							<button type="button" onClick="javascript:location.href='actividad.php?dt_DM=1&Cardcode=<?php echo base64_encode($row['CardCode']);?>&Contacto=<?php echo base64_encode($row['CodigoContacto']);?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']);?>&Direccion=<?php echo base64_encode($row['DireccionDestino']);?>&DM_type=<?php echo base64_encode('17');?>&DM=<?php echo base64_encode($row['DocEntry']);?>&dt_LS=1&LS=<?php echo base64_encode($row['ID_LlamadaServicio']);?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']);?>&pag=<?php echo base64_encode('orden_venta.php');?>'" class="alkin btn btn-primary pull-right m-l-xs"><i class="fa fa-plus-circle"></i> Agregar actividad</button>
						<?php }?>
			   ...
			   <form id="frm" action="" class="form-horizontal">
				...
				<div class="form-group">
					<div class="col-lg-9">
						...
						<?php }elseif($edit==1&&$row['Cod_Estado']=="O"&&PermitirFuncion(402)){?>
