...
if(isset($_GET['type'])&&$_GET['type']!=""){
	...
	if($_GET['type']==3){//Eliminar el archivo cargado por dropzone
	...
	if($_GET['type']==4){//Eliminar una linea del carrito en la Orden de venta
		if($_GET['edit']==1){
	...
	if($_GET['type']==5){//Consultar si ya existe el archivo de productos cargado en esa categoria
	...
	if($_GET['type']==6){//Eliminar una linea del carrito en la Oferta de venta
		if($_GET['edit']==1){
	...
	if($_GET['type']==7){//Limpiar carrito cuando estan creando un documento y selecciona el cliente
		...
		if($_GET['objtype']=="17"){//Orden de venta
	...
	if($_GET['type']==8){//Eliminar una linea del carrito en la Entrega de venta
		if($_GET['edit']==1){
	...
	if($_GET['type']==9){//Eliminar una linea del carrito en la Solicitud de salida
		if($_GET['edit']==1){
	...
	if($_GET['type']==10){//Eliminar una linea del carrito en la Salida de inventario
		if($_GET['edit']==1){
	...
	if($_GET['type']==11){//Agregar cantidades de lotes en los items de la Entrega de venta
	...
	if($_GET['type']==12){//Sumar el total de todos los lotes del item especificado
		$Total=SumarTotalLotesEntregar($_GET['itemcode'],$_GET['linenum'],$_GET['whscode'],$_GET['cardcode'],$_GET['objtype'],$_SESSION['CodUser']);
		echo $Total;
	}
	if($_GET['type']==13){//Eliminar una linea del carrito en la Traslado de inventario
		if($_GET['edit']==1){
	...
	if($_GET['type']==14){//Agregar al carrito de Facturacion de Orden de servicio
		$Parametros=array(
			"'".$_GET['metodo']."'",
			"'".$_GET['otsel']."'",
			"'".$_GET['vtaopt']."'",
		...
		$SQL=EjecutarSP('sp_tbl_FacturaOTDetalleCarritoInsert',$Parametros,-1);
		if($SQL){
			echo date('h:i:s a');
		}else{
	...
	if($_GET['type']==15){//Eliminar una linea del carrito en la Facturacion de Orden de servicio
		if($_GET['edit']==1){
			$Parametros=array(
				"'".$_GET['linenum']."'",
				"'".$_GET['cardcode']."'",
	...
	if($_GET['type']==16){//Consultar si existe el Socio de negocio (por cedula al crear)
		$Cons="Select CodigoCliente From uvw_Sap_tbl_Clientes Where LicTradNum LIKE '%".$_GET['id']."%'";
		$SQL_Cons=sqlsrv_query($conexion,$Cons);
		$row_Cons=sqlsrv_fetch_array($SQL_Cons);
	...
	if($_GET['type']==17){//Eliminar una linea del carrito en la Devolucion de venta
		if($_GET['edit']==1){
	...
	if($_GET['type']==18){//Agregar cantidades de seriales en los items
		if($_GET['edit']==1){//Creando documento
			$Parametros=array(
				"NULL",
				"NULL",
	...
	if($_GET['type']==19){//Sumar el total de todos los seriales del item especificado
		$Total=SumarTotalSerialesEntregar($_GET['itemcode'],$_GET['linenum'],$_GET['whscode'],$_GET['cardcode'],$_GET['objtype'],$_SESSION['CodUser']);
		echo $Total;
	}
	if($_GET['type']==20){//Eliminar una linea del carrito en la Factura de venta
		if($_GET['edit']==1){
	...
	if($_GET['type']==21){//Eliminar una linea en el cronograma de servicios
		$SQL_Del=Eliminar("tbl_ProgramacionOrdenesServicio","ID IN (".$_GET['linenum'].")");
		if($SQL_Del){
			echo "*Ok*";
		}
	}
	if($_GET['type']==22){//Agregar item en el cronograma
		$Parametros=array(
			"'".$_GET['cardcode']."'",
			"'".$_GET['idsucursal']."'",
			"'".$_GET['itemcode']."'",
	...
	if($_GET['type']==23){//Agregar item en las series de FE
		$Parametros=array(
			"'".$_GET['tipodoc']."'",
			"'".$_GET['series']."'",
			"'".$_SESSION['CodUser']."'"
	...
	if($_GET['type']==24){//Eliminar una linea en las series de FE
		$SQL_Del=Eliminar("tbl_FacturacionElectronica_Series","[ID]='".$_GET['linenum']."'",2);
		if($SQL_Del){
			echo "*Ok*";
		}
	}
	if($_GET['type']==25){//Eliminar una linea del carrito en el cierre de OT
		$Parametros=array(
			"'".$_GET['linenum']."'",
			"'".$_GET['tdoc']."'"
		);
	...
	if($_GET['type']==26){//Eliminar una linea del carrito en la creacion de OT
		$Parametros=array(
			"'".$_GET['linenum']."'"
		);
	...
	if($_GET['type']==27){//Duplicar una linea en el cronograma de servicios
		$Parametros=array(
			"'".$_GET['linenum']."'"
		);
	...
	if($_GET['type']==28){//Eliminar una linea en el cambio de producto
		$SQL_Del=Eliminar("tbl_CambioProductoOrdenVenta","[ID] IN (".$_GET['linenum'].")");
		if($SQL_Del){
			echo "*Ok*";
		}
	}
	if($_GET['type']==29){//Eliminar una linea en la gestion de series
		$SQL_Del=Eliminar("tbl_SeriesSucursalesAlmacenes","[ID]='".$_GET['linenum']."'");
		if($SQL_Del){
			echo "*Ok*";
		}
	}
	if($_GET['type']==30){//Agregar item a la gestion de series
		$Parametros=array(
			"'".$_GET['tipodoc']."'"
		);
	...
	if($_GET['type']==31){//Insertar o actualizar actividad desde el Programador de rutas
		$Parametros=array(
			"'".$_GET['sptype']."'",
			"'".$_GET['id_actividad']."'",
			"'".$_GET['id_evento']."'",
