...
function SubComent($pComents, $Len=100){//Substraer solo la cantidad de caracteres del string
	...
	$result = (strlen($pComents)>100) ? substr($pComents,1,100)."..." : $pComents;
...
function Eliminar($pVista, $pWhere='', $pType=1, $pDebugMode=0){//Eliminar datos de una tabla
	if($pType==1){//Consulta a SQL SERVER
		global $conexion;
...
function FormatoNombreAnexo($pAnexo){//Cambia el nombre del anexo para guardarlo
...
function EnviarWebServiceSAP($pNombreWS,$pParametros,$pJSON=false,$pAPI=false){
...
function SumarTotalLotesEntregar($ItemCode, $LineNum, $WhsCode, $CardCode, $Objtype, $Usuario){//Sumar el total que se está entregando en los lotes de SAP B1
	global $conexion;
	$Parametros=array(
		"'".$ItemCode."'",
		"'".$LineNum."'",
