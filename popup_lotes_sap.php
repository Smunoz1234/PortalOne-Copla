...
if(isset($_GET['docentry'])&&($_GET['docentry']!="")){
	...
	if($edit==1){//Creando documento
		...
		if($objtype==15){//Entrega de ventas
			$SQL_Items=Seleccionar("uvw_tbl_EntregaVentaDetalleCarrito","*","Usuario='".$_GET['usuario']."' and CardCode='".$_GET['cardcode']."' and WhsCode='".$_GET['whscode']."' and ManBtchNum='Y'");
		}elseif($objtype==16){//Devolucion de ventas
			$SQL_Items=Seleccionar("uvw_tbl_DevolucionVentaDetalleCarrito","*","Usuario='".$_GET['usuario']."' and CardCode='".$_GET['cardcode']."' and WhsCode='".$_GET['whscode']."' and ManBtchNum='Y'");
		}		
...
function BuscarLote(item,almacen,numlinea,itemname,und,cant,basetype,base_entry,baseline){
	...
	$.ajax({
		...
		url: "ajx_lotes_articulos.php?id="+item+"&cardcode=<?php echo $_GET['cardcode'];?>&whscode=<?php echo $_GET['whscode'];?>&usuario=<?php echo $_GET['usuario'];?>&edit=<?php echo $edit;?>&objtype=<?php echo $objtype;?>&linenum="+numlinea+"&itemname="+itemname+"&und="+und+"&cant="+cant.replace(/,/g, '')+"&basetype="+basetype+"&base_entry="+base_entry+"&baseline="+baseline+"&sentido=<?php echo $sentido;?>",
		<?php }else{?>
