...
$Cons="Select * From uvw_Sap_tbl_EntregasVentas Where (DocDate Between '$FechaInicial' and '$FechaFinal') $Filtro Order by DocNum DESC";
...
if(isset($_GET['IDTicket'])&&$_GET['IDTicket']!=""){
	...
	$Cons="Select * From uvw_Sap_tbl_EntregasVentas Where $Where";
}
...
<title>Consultar entrega | <?php echo NOMBRE_PORTAL;?></title>
<!-- InstanceEndEditable -->
        ...
        <div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-sm-8">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">
                            <strong>Consultar entrega</strong>
                        </li>
                    </ol>
                </div>
            </div>
         <div class="wrapper wrapper-content">
             <div class="row">
			    ...
			    <div class="ibox-content">
				  ...
				  <form action="consultar_entrega_venta.php" method="get" id="formBuscar" class="form-horizontal">
                    ...
                    <?php
						...
						while($row=sqlsrv_fetch_array($SQL)){ ?>
						 <tr class="gradeX">
							...
							<td><?php if($row['ID_LlamadaServicio']!=0){?><a href="llamada_servicio.php?id=<?php echo base64_encode($row['ID_LlamadaServicio']);?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']);?>&pag=<?php echo base64_encode('consultar_entrega_venta.php');?>&tl=1" target="_blank"><?php echo $row['DocNumLlamadaServicio'];?></a><?php }else{echo "--";}?></td>
							...
							<td><a href="entrega_venta.php?id=<?php echo base64_encode($row['ID_EntregaVenta']);?>&id_portal=<?php echo base64_encode($row['IdDocPortal']);?>&tl=1&return=<?php echo base64_encode($_SERVER['QUERY_STRING']);?>&pag=<?php echo base64_encode('consultar_entrega_venta.php');?>" class="alkin btn btn-success btn-xs"><i class="fa fa-folder-open-o"></i> Abrir</a> <a href="sapdownload.php?id=<?php echo base64_encode('15');?>&type=<?php echo base64_encode('2');?>&DocKey=<?php echo base64_encode($row['ID_EntregaVenta']);?>&ObType=<?php echo base64_encode('15');?>&IdFrm=<?php echo base64_encode('0');?>" target="_blank" class="btn btn-warning btn-xs"><i class="fa fa-download"></i> Descargar</a></td>
						</tr>
					<?php }
						}?>
                    </tbody>
