...
if($sw==1){
	...
	$SQL_VTAOpcional=Seleccionar('uvw_Sap_tbl_ArticulosVenta','ItemCode, ItemName',"CodigoCliente='".NIT_EMPRESA."'",'ItemName');
	
	//Consultar si se adiciona el 30%
	$SQL_Adicional30=Seleccionar('uvw_Sap_tbl_Clientes','AplicaIVAExcluido',"CodigoCliente='".$Cliente."'");
	$row_Adicional30=sqlsrv_fetch_array($SQL_Adicional30);
					...
					<div class="col-lg-4">
						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu">
								...
								<li><a class="dropdown-item" href="#" id="btnIndBySuc" title="No agrupa las sucursales, sino que agrupa por los VTA, si estos se repiten, suma la cantidad de OT y valores del VTA en la misma fila"><strong>NO</strong> Agrupar por sucursal</a></li>
							</ul>
						</div>	
						<div class="btn-group">
           ...
           <div class="col-lg-12">
			    <div class="ibox-content">
					...
					<div class="row p-md">
						...
						<div class="col-lg-2">
							<label class="control-label">Agregar el 30% en la factura</label>
							<select name="Agregar30" class="form-control" id="Agregar30">
								<option value="SI" <?php if(isset($row_Adicional30['AplicaIVAExcluido'])&&($row_Adicional30['AplicaIVAExcluido']=="SI")){ echo "selected=\"selected\"";}?>>SI</option>
								<option value="NO" <?php if(isset($row_Adicional30['AplicaIVAExcluido'])&&($row_Adicional30['AplicaIVAExcluido']=="NO")){ echo "selected=\"selected\"";}?>>NO</option>
							</select>	
						</div>
						<div class="col-lg-1">
...
<!-- InstanceBeginEditable name="EditRegion4" -->
 <script>
        $(document).ready(function(){
			...
			$('#FechaInicialActividad').datepicker({
                todayBtn: "linked",
                keyboardNavigation: false,
                forceParse: false,
                calendarWeeks: true,
			 ...
			 $('#FechaFinalActividad').datepicker({
                todayBtn: "linked",
                keyboardNavigation: false,
                forceParse: false,
                calendarWeeks: true,
...
function CopiarToFactura(adj=1,dest=1){
	...
	let Agregar30 = document.getElementById('Agregar30');	
	...
	if(CodClienteFactura.value!=""){
		window.location = docDest+"?dt_FC=1&Cardcode=<?php echo base64_encode($_GET['Cliente']);?>&Sucursal=<?php echo base64_encode($_GET['Sucursal']);?>&adt="+btoa(adj)+"&CodFactura="+btoa(CodClienteFactura.value)+"&add30="+btoa(Agregar30.value);
	}else{
		window.location = docDest+"?dt_FC=1&Cardcode=<?php echo base64_encode($_GET['Cliente']);?>&Sucursal=<?php echo base64_encode($_GET['Sucursal']);?>&adt="+btoa(adj)+"&CodFactura=&add30="+btoa(Agregar30.value);
	}	
}
</script>
<!-- InstanceEndEditable -->
