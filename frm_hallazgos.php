...
if(isset($_POST['P'])&&($_POST['P']==base64_encode('MM_frmHallazgos'))){
	...
	try{
		...
		$ParamInsFrm=array(
			...
			"'".LSiqmlObs($_POST['ResponsableCliente'])."'",
		...
		if($SQL_InsFrm){
			...
			sqlsrv_close($conexion);
			if($_POST['tl']==1){
				header('Location:'.base64_decode($_POST['return']).'&a='.base64_encode("OK_FrmUpd").'&new='.base64_encode($ID));
			}else{
				header('Location:'.base64_decode($_POST['return']).'&a='.base64_encode("OK_FrmAdd").'&new='.base64_encode($IdFrm));	
			}			
		}else{
			$sw_error=1;
...
<script type="text/javascript">
	$(document).ready(function() {//Cargar los combos dependiendo de otros
		...
		$("#SucursalCliente").change(function(){
			...
			$.ajax({
				type: "POST",
				url: "ajx_cbo_select.php?type=4&id="+Cliente+"&suc="+Base64.encode(Sucursal),
				async:false,
				success: function(response){
				...
				<div class="col-lg-12">   		
					...
					<?php include("includes/spinner.php"); ?>
						<div class="form-group">
							<div class="col-lg-6">
								<a href="sapdownload.php?id=<?php echo base64_encode('13');?>&type=<?php echo base64_encode('2');?>&DocKey=<?php echo base64_encode($row['ID_Frm']);?>" target="_blank" class="btn btn-outline btn-success"><i class="fa fa-download"></i> Descargar formato</a>
							</div>			
						</div>
					</div>
				</div>
           ...
           <div class="col-lg-12">
              <form action="frm_hallazgos.php" method="post" class="form-horizontal" enctype="multipart/form-data" id="CrearFrm">
				...
				<div class="form-group">
					...
					<div class="col-lg-3">
                    	<select name="ContactoCliente" class="form-control m-b" id="ContactoCliente" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
					...
					<label class="col-lg-1 control-label">Sucursal</label>
				  	<div class="col-lg-3">
                    	<select name="SucursalCliente" class="form-control m-b select2" id="SucursalCliente" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
						  <?php if((($type_frm==0)||($sw_error==1))&&($dt_LS!=1)){?><option value="">Seleccione...</option><?php }?>
				...
				<div class="form-group">
					<label class="col-lg-1 control-label"><?php if(($type_frm==1)&&($row['ID_LlamadaServicio']!=0)){?><a href="llamada_servicio.php?id=<?php echo base64_encode($row['ID_LlamadaServicio']);?>&tl=1" target="_blank" title="Consultar Llamada de servicio" class="btn-xs btn-success fa fa-search"></a> <?php }?>Orden servicio</label>
				  	<div class="col-lg-4">
						<select name="OrdenServicio" class="form-control m-b select2" id="OrdenServicio" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
				...
				<div class="form-group">
					<label class="col-lg-1 control-label">Área</label>
				  	<div class="col-lg-3">
                    	<select name="Area" class="form-control m-b select2" required="required" id="Area" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
					...
					<div class="col-lg-2">
                    	<select name="TipoVisita" class="form-control m-b" required="required" id="TipoVisita" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
								<option value="">Seleccione...</option>
					...
					<div class="col-lg-2">
                    	<select name="Estado" class="form-control m-b" id="Estado" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
				...
				<div class="form-group">
					...
					<div class="col-lg-4">
                    	<select name="Empleado" class="form-control m-b select2" id="Empleado" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
					...
					<div class="col-lg-2">
                    	<select name="Revision" class="form-control m-b" id="Revision" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
					...
					<div class="col-lg-2">
                    	<select name="EstadoCriticidad" required="required" class="form-control m-b" id="EstadoCriticidad" <?php if(($type_frm==1)&&($row['Cod_Estado']=='-1')){ echo "disabled='disabled'";}?>>
							<option value="">Seleccione...</option>
                           <?php while($row_EstadoCriticidad=sqlsrv_fetch_array($SQL_EstadoCriticidad)){?>
								<option value="<?php echo $row_EstadoCriticidad['IdEstadoCriticidad'];?>" <?php if((isset($row['IdEstadoCriticidad']))&&(strcmp($row_EstadoCriticidad['IdEstadoCriticidad'],$row['IdEstadoCriticidad'])==0)){ echo "selected=\"selected\"";}?>><?php echo $row_EstadoCriticidad['DeEstadoCriticidad'];?></option>
						  <?php }?>
						</select>
           	  	  </div>
				</div>
