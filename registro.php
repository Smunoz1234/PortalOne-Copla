...
if($P==36){//Actualizar los datos del detalle de los documentos de SAP
	try{
		...
		if($_GET['doctype']==4){//Solicitud de traslado
			if($_GET['type']==1){//Actualiza campos en carrito
				$Parametros=array(
					"'".$_GET['name']."'",
					"'".base64_decode($_GET['value'])."'",
		...
		if($_GET['doctype']==6){//Traslado de inventario
			if($_GET['type']==1){//Actualiza campos en carrito
				$Parametros=array(
					"'".$_GET['name']."'",
					"'".base64_decode($_GET['value'])."'",
		...
		if($_GET['doctype']==7){//Devolucion de venta
			if($_GET['type']==1){//Actualiza campos en carrito
				$Parametros=array(
					"'".$_GET['name']."'",
					"'".base64_decode($_GET['value'])."'",
		...
		if($_GET['doctype']==8){//Facturacion de OT
			if($_GET['type']==1){//Actualiza campos en carrito
				$Parametros=array(
					"'".$_GET['name']."'",
					"'".base64_decode($_GET['value'])."'",
		...
		if($_GET['doctype']==9){//Factura de venta
			if($_GET['type']==1){//Actualiza campos en carrito
				$Parametros=array(
					"'".$_GET['name']."'",
					"'".base64_decode($_GET['value'])."'",
		...
		if($_GET['doctype']==10){//Actualiza el detalle del cierre de OT
			$Parametros=array(
				"'".$_GET['name']."'",
		...
		if($_GET['doctype']==11){//Actualiza el detalle del cronograma de servicios
			$Parametros=array(
				"'".$_GET['name']."'",
				"'".FormatoFecha(base64_decode($_GET['value']))."'",
				"'".$_GET['line']."'",
		...
		if($_GET['doctype']==12){//Actualiza el detalle de las series de FE
			$Parametros=array(
				"'".$_GET['name']."'",
				"'".FormatoFecha(base64_decode($_GET['value']))."'",
				"'".$_GET['line']."'",
			...
			$SQL=EjecutarSP('sp_tbl_FacturacionElectronica_SeriesUpdCampos',$Parametros,36,2);
			if($SQL){
				sqlsrv_close($conexion);
				echo date('h:i:s a');
			}else{
		...
		if($_GET['doctype']==13){//Actualiza los campos de la gestion de series
			$Parametros=array(
				"'".$_GET['name']."'",
				"'".base64_decode($_GET['value'])."'",
				"'".$_GET['line']."'",
		...
		if($_GET['doctype']==14){//Actualiza los datos de la creacion de actividades en lote en el programador
			$Parametros=array(
				"'".$_GET['name']."'",
				"'".base64_decode($_GET['value'])."'",
				"'".$_GET['line']."'",
		...
		if($_GET['doctype']==15){//Actualiza los datos en el detalle de la Lista de materiales
			$Parametros=array(
				"'".$_GET['name']."'",
...
if($P==52){//Traslado de inventario
}
	
if($P==53){//Contratos
}
	
if($P==54){//Devolucion de venta
}
	
if($P==55){//Factura de venta
}
	
if($P==56){//Parametros de Facturación electronica
}
	
if($P==57){//Solicitud de compra
}
	
if($P==58){//Entrada de compra
}
