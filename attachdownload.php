...
if(isset($_REQUEST['file'])&&$_REQUEST['file']!=""){
	...
	if($type==1){
		...
		if($ZipMode==1){//Comprimir los archivos descargados
			...
			$i=0;
			$j=0;//indici interno del array Files, por si algun registro viene sin nombre
			while($i<$Count){
			...
			if($zip->open($filezip, ZIPARCHIVE::CREATE)===TRUE){
				...
				while($i<$Count){
					...
					while($row=sql_fetch_array($SQL,2)){
						...
						$zip->addFile($RutaLocal.$row['NombreArchivo'],$archivos[$i]['LicTradNum']."/".$row['DeCategoria']."/".$row['NombreArchivo']);					
